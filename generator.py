...
class NoisyImageGenerator(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        x = np.zeros((batch_size, int(image_size[1]), int(image_size[0]), 3), dtype=np.uint8)
        y = np.zeros((batch_size, int(image_size[1]), int(image_size[0]), 3), dtype=np.uint8)
        sample_id = 0
        ...
        while True:
            ...
            if h >= int(image_size[0]) and w >= int(image_size[1]):
                h, w, _ = image.shape

                #i = np.random.randint(h - image_size[0] + 1)
                #j = np.random.randint(w - image_size[1] + 1)
                #print("i, j")
                #print(i, j)
                #exit(1)
                #clean_patch = image[i:i + image_size, j:j + image_size]
                clean_patch = image[h - int(image_size[1]):h, w - int(image_size[0]):w]
                if np.shape(self.source_noise_model(clean_patch))[1] < 234:
                    continue
                x[sample_id] = self.source_noise_model(clean_patch)
                y[sample_id] = self.target_noise_model(clean_patch)

                #x[sample_id] = self.source_noise_model(image)
                #y[sample_id] = self.target_noise_model(image)
